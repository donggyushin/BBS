...
<html>
  ...
  <%
  	...
  	if(bbsID == 0){
  		PrintWriter script = response.getWriter();
  		script.println("<script>");
  		script.println("alert('can't access this page')");
  	...
  	if(!userID.equals(bbs.getUserID())){
  		PrintWriter script = response.getWriter();
  		script.println("<script>");
  		script.println("alert('you don't have authorize')");
  		script.println("history.back()");
  		script.println("</script>");
  	}
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    ...
    <form method="post" action="updateAction.jsp?bbsID=<%= bbsID %>">
    	<table class="table">
			  <thead>
			    <tr>
			      <th scope="col" colspan="2" class="text-center">bbs editing form</th>
			    </tr>
			  </thead>
			  <tbody>
			    <tr>
			      
			      <td><input type="text" name="bbsTitle" placeholder="title" class="form-control" maxlength="50" value="<%= bbs.getBbsTitle() %>"></td>
			    </tr>
			    <tr>
			      <td><textarea  name="bbsContent" placeholder="Content" class="form-control" maxlength="2048" style="height:350px"><%=bbs.getBbsContent() %></textarea></td>
			...
			<input id="write" type="submit" class="btn btn-primary pull-right" value="Edit">
    </form>
